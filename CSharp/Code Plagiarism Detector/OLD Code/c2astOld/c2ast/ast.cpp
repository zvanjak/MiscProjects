...
bool AST::GetTypeString(AST_node &parent, source &src, string &typestring) {
	...
	if (pt != "struct" && pt != "enum" && !parent.IsType(pt)) {
		src.UnGetPToken();
		return false;
	}

	if (pt == "struct" || pt == "enum") {
		c_type ct;
		c_decl cd;
	...
	do {
		...
		if (pt == "}") {
			...
			if (pt == "if") {
				...
				ifnod.type = AST_if_true;
				ProcessBlock(ifnod, src);
				nod.AddBlock(ifnod);

				if (src.GetNextPToken() == "else") {
					AST_node elsenode(&nod);
					elsenode.type = AST_if_false;
					ProcessBlock(elsenode, src);
					nod.AddBlock(elsenode);
				} else {
					src.UnGetPToken();
				}
			} else {
		...
		if (wasspecial) {
			if (!komad.empty()) {
				AST_node bnod(&parent);
				bnod.type = AST_block;
				bnod.src = komad;
				komad.clear();
				parent.AddBlock(bnod);
			}
			parent.AddBlock(nod);
 ...
 *  AST_node        *
 ********************/
AST_node::AST_node(AST_node *_parent) {
	parent = _parent;
}
...
void AST_node::AddBlock(const AST_node &an) {
...
string AST_node::TypeToString(AST_node_type type) {
	switch (type) {
		...
		case AST_function_code: return "function code";
		case AST_if:		return "if";
		case AST_if_true:	return "if -> true";
		case AST_if_false:	return "if -> false";
