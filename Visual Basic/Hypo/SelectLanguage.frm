...
Private Sub Form_Load()
    ...
    ' open table 'Employee' from database and get data about employees
    cnnEmp.ConnectionString = "DSN=Hypo;"
    cnnEmp.Open
   
    rstEmp.ActiveConnection = cnnEmp
    rstEmp.Source = "SELECT * FROM Employee ORDER BY surname"
    ...
    Dim pom As String
    Dim s As String
    Dim pom_ind As Integer
    Dim korak As Integer
    
    korak = EmpNum / 2
    Do While korak > 0
        For i = korak + 1 To EmpNum - 1
            pom = UCase(arrEmp(arrIndSortName(i)).Name)
            pom_ind = arrIndSortName(i)
            
            j = i
            Do
                s = UCase(arrEmp(arrIndSortName(j - korak)).Name)
                If pom < s Then
                    arrIndSortName(j) = arrIndSortName(j - korak)
                Else
                    Exit Do
                End If
                j = j - korak
